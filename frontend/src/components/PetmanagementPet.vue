<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            Pet # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            Pet
        </v-card-title >        
        ...
        <v-card-actions>
            ...
            <v-dialog v-model="feedDiagram" width="500">
                <FeedCommand
                    @closeDialog="closeFeed"
                    @feed="feed"
                ></FeedCommand>
            </v-dialog>
            <v-btn
            ...
            <v-dialog v-model="sleepDiagram" width="500">
                <SleepCommand
                    @closeDialog="closeSleep"
                    @sleep="sleep"
                ></SleepCommand>
            </v-dialog>
            <v-btn
            ...
            <v-dialog v-model="groomDiagram" width="500">
                <GroomCommand
                    @closeDialog="closeGroom"
                    @groom="groom"
                ></GroomCommand>
            </v-dialog>
        </v-card-actions>
	...
	async created() {
        ...
        methods: {
            ...
            async feed(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['/feed'].href), params)
            ...
            openFeed() {
                this.feedDiagram = true;
            },
            closeFeed() {
                this.feedDiagram = false;
            },
            async sleep(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['/sleep'].href), params)
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
            ...
            openSleep() {
                this.sleepDiagram = true;
            },
            closeSleep() {
                this.sleepDiagram = false;
            },
            async groom(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['/groom'].href), params)
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
            ...
            openGroom() {
                this.groomDiagram = true;
            },
            closeGroom() {
                this.groomDiagram = false;
            },
        },
    }
</script>
