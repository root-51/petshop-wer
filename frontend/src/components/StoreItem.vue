<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            Item # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            Item
        </v-card-title >        
        ...
        <v-card-text style="background-color: white;">
            ...
            <User offline label="Customer" v-model="value.customer" :editMode="editMode" @change="change"/>
